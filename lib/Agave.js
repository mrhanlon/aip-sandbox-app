...
(function( window, $, SwaggerApi, authorizations, PasswordAuthorization, ApiKeyAuthorization, undefined ) {
    ...
    if ( agaveToken ) {
      try {
        ...
        if ( Date.now() - agaveToken.created < ( 1000 * agaveToken.expires_in ) ) {
          ...
          Agave.auth.refreshToken( agaveToken.refresh_token )
          .done(function( token ) {
  ...
  Agave.initialize = function() {
    ...
    if (Agave.client && Agave.token) {
      authorizations.add( 'Authorization', new ApiKeyAuthorization( 'Authorization', 'Bearer ' + Agave.token.access_token, 'header' ) );

...
(function(window, $, SwaggerApi, ApiKeyAuthorization, undefined){
  ...
  showToken = function showToken() {
    $('.current-token').html('<p>Value: <span class="label label-info">'+ Agave.oauth2.access_token +'</span> Expires: ' + new Date(Agave.oauth2.created + (1000 * Agave.oauth2.expires_in)).toLocaleString() + '</p>');
